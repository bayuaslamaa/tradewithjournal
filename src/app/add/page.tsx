    ...
    <div className="max-w-2xl mx-auto pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div className="flex items-center gap-4">
          <a href="/" className="w-10 h-10 flex items-center justify-center rounded-xl transition-all hover:bg-[var(--surface-hover)] group"
            style={{ background: 'var(--surface2)', border: '1px solid var(--border)' }}>
            <span className="mono text-[var(--muted)] group-hover:text-[var(--text)] transition-colors text-lg mt-[-2px]">←</span>
          </a>
          <div>
            <h1 className="display-font font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-[var(--muted)]" style={{ fontSize: '1.8rem', letterSpacing: '-0.02em' }}>
              Log a Trade
            </h1>
      ...
      <div className="grid gap-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {(['LONG', 'SHORT'] as Direction[]).map(d => (
                <button key={d} onClick={() => set('direction', form.direction === d ? null : d)}
                  className="mono py-3.5 rounded-xl font-bold transition-all outline-none focus-visible:ring-2 focus-visible:ring-white/20"
                  style={{
        ...
        <Section label="03 — BEFORE THE TRADE">
          <SuggestField
            label="REASON / SETUP"
            required
            placeholder="Why are you taking this trade? Describe the setup, R:R, confluence…"
            value={form.reason ?? ''}
          ...
          <Field label="AI OUTLOOK" accent>
            <div className="relative">
              ...
              <textarea rows={3} placeholder="Paste what your AI said about this setup…"
                value={form.aiOutlook ?? ''} onChange={e => set('aiOutlook', e.target.value)}
                className="glass-input form-input resize-none pr-10" style={{ borderColor: 'rgba(0,255,178,0.2)' }} />
            </div>
          </Field>

          <SuggestField
            label="EMOTION BEFORE"
            placeholder="How are you feeling RIGHT NOW before entering?"
            rows={2}
        ...
        <Section label="04 — AFTER THE TRADE">
          ...
          <SuggestField
            label="EMOTION AFTER"
            placeholder="How did you feel after the trade closed?"
            rows={2}
            ...
            <div className="grid grid-cols-5 gap-2 sm:gap-3">
              {[1,2,3,4,5].map(n => (
                <button key={n} onClick={() => set('grade', form.grade === n ? null : n)}
                  className="mono py-3 sm:py-4 rounded-xl font-bold transition-all outline-none"
                  style={{
                    fontSize: '1rem',
          ...
          <Field label="LESSON (1 sentence)">
            <textarea rows={2} placeholder="What's the one thing you learned from this trade?"
        ...
        <div className="pt-4">
          <button onClick={handleSubmit} disabled={loading}
            ...
            style={{
              background: loading ? 'var(--surface2)' : 'linear-gradient(135deg, var(--accent) 0%, #00b377 100%)',
